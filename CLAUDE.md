...
- **Payment methods**: the `metodos_pago` catalog already existed in production (drift — now reconciled into `db/schema.sql`): `efectivo/transferencia/tarjeta/paypal/mercadopago/oxxo` with `activo`/`requiere_link`/`configuracion`(JSON, e.g. CLABE). `pedidos.metodo_pago` (migration 0014) records the per-order choice. Managed via `GET /api/metodos-pago` + `PUT /api/metodos-pago/:id` (prime) and selected during onboarding. **The in-bot customer-facing payment-method picker is a pending follow-up** — JC's live checkout still generates the simulated link only (`marcar-pagado` remains the single chokepoint all methods converge on). "Pago real" (gateway) stays stubbed on purpose.
