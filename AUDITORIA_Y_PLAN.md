...
- [x] 2026-07-09 (v1.04) **Backup cifrado + restaurar desde Prime**: services/cryptoBackup.js (PBKDF2 210k iter + AES-256-GCM). Modo ALTO = maestra no se guarda, la clave derivada se MUESTRA una vez (apuntar), vive en RAM (armar tras reinicio); modo BAJO = clave aleatoria envuelta con el secreto de instancia en la BD (auto); OFF = gz en claro. backup.js cifra según modo (alto recibe la clave por env del respaldo manual). Restaurar: sube el .enc/.gz, valida SQLite, hace SWAP-ON-BOOT en db_connection (nunca reemplaza archivo abierto), conserva .pre-restore-*, NO toca .wwebjs_auth ni .instancia_secret. Probado round-trip completo. OJO: modo alto sin la maestra+clave apuntada = irrecuperable (por diseño); scheduled cron en alto no cifra (sin clave armada) — usa bajo para auto-cifrado 24/7
- [x] 2026-07-09 (v1.05) **Pasarela — link de pago** (toggle pago_link_activo): services/pagoLinkService.js es el punto único (enganche del gateway: env PAGO_GATEWAY+KEY, hoy stub). Sin gateway usa el link que el negocio YA tiene (Clip/MP/PayPal.me, configurable en Prime>General) + referencia del pedido. Se envía por WhatsApp desde el bot (checkout) y desde el POS/Pedidos (botón enviar-link). El cobro real sigue confirmándose en el chokepoint marcar-pagado. Probado: sin link=error claro, con link=anexa ref respetando query
- [x] 2026-07-09 (v1.06) **Recompra — las dos** (módulo recompra_activo, default OFF): (a) COMPRAS: botón "Reordenar" en cada OC → crea una OC nueva copiando los items (proveedores e insumos); (b) CLIENTE: checkRecompraConsumibles en el watcher — a quien compró un producto tipo consumible hace ~30 días (config recompra_dias) y no ha vuelto, el bot le recuerda recomprarlo (dedupe 20 días, respeta opt-out). Probado el SQL: solo consumibles en ventana
- [x] 2026-07-09 (v1.07) **Nómina fiscal (LFT)** (módulo nomina_fiscal_activo, default OFF=sencilla): expediente con fecha_alta/antigüedad, CURP, departamento, comisión %, método de pago, usuario POS. Nómina fiscal = horas extra (>8h/día al DOBLE) + comisiones (% sobre lo cobrado por el empleado vía cobrado_por) + ISR/IMSS. Funciones LFT: antigüedad, vacaciones (12+2/año), aguinaldo proporcional, finiquito (partes proporcionales + opción indemnización 3m+20d/año). Endpoints aguinaldo/finiquito + botones en RRHH. Al pagar genera asiento (601/102). Probado: aritmética LFT + calcular fiscal exacto. Disclaimer "aproximado, valida con contador" persiste
- [x] 2026-07-09 (v1.03) **Reporte de ventas por producto** (dueño): ERP > Ventas por producto — qué/cuánto/en cuánto, funciona sin inventario; el negocio informal tiene su reporte para formalizarse
- [x] 2026-07-10 (v1.08) **Restaurante/mesas** (módulo mesas_activo, default OFF): página Mesas — abrir mesas, agregar platillos del catálogo o libres CON comentario (sin chile/mitad), "enviar a cocina" genera la comanda (preticket) y marca los items, cobrar cierra reusando la maquinaria del POS (insertarPedidoConCarrito + links_pago pagado + libera la mesa). Rutas dashboard/routes/mesas.js (área pos). Probado el flujo completo. De paso: arreglado el link de Citas que estaba oculto (faltaba citas_activo en el mapa de módulos del Layout)
