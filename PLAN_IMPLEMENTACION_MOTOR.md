...
4. **Migración = la siguiente libre al construir** (regla anti-drift: NO fijar un número en el diseño).
   Al 2026-07-12 la última aplicada es `0063_documentos.sql`, así que el motor sería **≥0064** —
...
1. **Última migración aplicada = `0063_documentos.sql`** (`ls migrations/ | sort | tail -1`).
   Por tanto la migración del motor es **`0064_flujo_motor.sql`** — verifica `tail -1` de nuevo al
   ...
   léase `0061`. `PLAN` §Fase 2+ punto 4 ya dice ≥0064 — correcto.
...
- **Migración = siguiente libre = `0064_flujo_motor.sql`** (verificar `tail -1` al construir).
...
1. **`DISENO_MOTOR_FLUJO.md` — número de migración obsoleto.** El encabezado dice "hoy ≥`0059`",
   ...
   aplicado real = `0063_documentos.sql`, hoy sería `0064`). El `PLAN` §Fase 2+ punto 4 ya está correcto.
