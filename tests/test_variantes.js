...
let pass = 0, fail = 0;
const ok = (c, m) => { if (c) { pass++; console.log('  ok ' + m); } else { fail++; console.error('  FALLO ' + m); } };
...
// 1. guardar matriz con stock en 2 sucursales
vs.guardarMatriz(1, [
...
// 2. variantesConStock para el bot
...
// 3. escáner POS por UPC de variante
const porUpc = vs.porCodigo('750VAR0002');
ok(porUpc && porUpc.id_producto === 1 && porUpc.talla === 'L', 'UPC de variante resuelve producto+talla');

// 4. venta descuenta el espejo de la variante
...
// 5. quitar una fila la INACTIVA y recalcula agregado
...
// 6. carrito distingue variantes (misma prenda, tallas distintas = 2 renglones)
const carrito = [];
const items = [
