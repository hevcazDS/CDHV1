...
// teléfono/nombre, cuenta visitas del mes, y lista el día.  node --test tests/test_asistencias.js
...
test('check-in por teléfono resuelve el cliente y cuenta 1 visita del mes', () => {
    const out = {};
...
test('segundo check-in del mismo socio → 2 visitas del mes', () => {
    const out = {};
    A.checkin({ _body: { id_cliente: idCli } }, null, ctx(out), { ses: {} });
    assert.strictEqual(out.d.visitas_mes, 2);
});

test('check-in de visitante sin cuenta (solo nombre)', () => {
    const out = {};
...
test('listar el día devuelve las 3 visitas', () => {
    const out = {};
