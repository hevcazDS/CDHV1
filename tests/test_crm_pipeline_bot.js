...
//   node --test tests/test_crm_pipeline_bot.js
...
test('avanzarEtapa: lead/null → contactado → cotizado → ganado, con log en crm_etapas', () => {
...
test('NO retrocede ni repite (idempotente)', () => {
...
test('un GANADO no lo degrada el bot (ni a perdido)', () => {
...
test("'perdido' solo explícito (permitirPerdido)", () => {
...
test('gating: crm_pipeline_activo OFF → no-opea todo', () => {
...
test('nota + score suben en la ficha', () => {
...
test('INTEGRACIÓN: primer mensaje del cliente → contactado (sin mensajes falsos)', async () => {
...
test('INTEGRACIÓN: la acción cotizar del motor marca cotizado + sube score', () => {
...
test('P2 saludo: un cliente GANADO recibe el saludo frecuente; un lead, el normal', () => {
