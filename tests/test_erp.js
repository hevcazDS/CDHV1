...
conta._setDb(db); costeo._setDb(db); nomina._setDb(db);
conta._setActivo(() => true);

// 1. Asiento manual cuadrado
test('asiento manual cuadrado se registra', () => {
    const id1 = conta.registrarAsiento({ concepto: 'Capital inicial', partidas: [
        { cuenta: '102', debe: 1000 }, { cuenta: '301', haber: 1000 },
    ]});
    assert.ok(Number.isInteger(id1), 'asiento cuadrado se registra');
});

// 2. Asiento descuadrado se rechaza
test('asiento descuadrado se rechaza', () => {
    let rechazado = false;
    try { conta.registrarAsiento({ concepto: 'Malo', partidas: [{ cuenta: '102', debe: 100 }, { cuenta: '301', haber: 99 }] }); }
    catch (_) { rechazado = true; }
    assert.ok(rechazado, 'asiento descuadrado se rechaza');
});

// 3. Costeo promedio ponderado: stock 10 a $40 + entrada 10 a $60 → $50
test('costeo promedio ponderado: 10×40 + entrada 10×60 → 50, actualiza productos.costo e historial', () => {
    const r = costeo.registrarEntrada(1, 10, 60, 'test');
    assert.ok(r.costo_promedio === 50, `promedio ponderado 10×40 + 10×60 → 50 (dio ${r.costo_promedio})`);
    assert.ok(db.prepare('SELECT costo FROM productos WHERE id=1').get().costo === 50, 'productos.costo actualizado');
    assert.ok(db.prepare('SELECT COUNT(*) c FROM historial_costos').get().c === 1, 'historial registrado');
});

// 4. Libro mayor cuadra (suma debe == suma haber)
test('libro mayor cuadra (suma debe == suma haber)', () => {
    ...
    assert.ok(Math.abs(debe - haber) < 0.01, 'libro mayor cuadrado');
});

// 5. Reversa idempotente: asiento de venta + doble reversa = solo 1 reversa
test('reversa idempotente: asiento de venta + doble reversa = solo 1 reversa', () => {
    ...
    assert.ok(reversas === 1, 'reversa idempotente (1 sola aunque se llame 2 veces)');
});
...
test('cierre TOTAL de período: bloquea sin override, admite override con backdate, mes abierto no requiere override', () => {
    ...
    assert.ok(cerroBloqueo, 'mes cerrado bloquea sin override');
    ...
    assert.ok(Number.isInteger(idOv), 'mes cerrado admite override autorizado');
    assert.ok(db.prepare('SELECT fecha FROM asientos WHERE id=?').get(idOv).fecha === '2025-01-15', 'backdate respeta la fecha capturada');
    ...
    assert.ok(Number.isInteger(idAbierto), 'mes abierto no requiere override');
...
test('aguinaldo: asiento 601/102 idempotente por (empleado, año); sin contabilidad activa se registra sin asiento', () => {
    const emp = { id: 7, nombre: 'Ana', salario_diario: 400 };
    const ag = nomina.pagarAguinaldo(emp, 2025, 6000, 'gerente1');
    assert.ok(Number.isInteger(ag.id_asiento) && ag.total === 6000, 'aguinaldo asienta el monto (contabilidad on)');
    assert.ok(db.prepare("SELECT usuario FROM nomina_extraordinaria WHERE referencia='aguinaldo_7_2025'").get().usuario === 'gerente1', 'aguinaldo deja registro permanente con usuario');
    let agDup = false;
    try { nomina.pagarAguinaldo(emp, 2025, 6000, 'gerente1'); } catch (_) { agDup = true; }
    assert.ok(agDup, 'aguinaldo idempotente (no se paga dos veces el mismo año)');
    ...
    assert.ok(ag2.id_asiento === null && ag2.asentado_contable === false && ag2.total === 4500, 'aguinaldo sin contabilidad: pagado y registrado, sin asiento');
    assert.ok(!!db.prepare("SELECT 1 FROM nomina_extraordinaria WHERE referencia='aguinaldo_8_2025'").get(), 'aguinaldo sin contabilidad queda en nomina_extraordinaria');
    conta._setActivo(() => true);
});
...
test('venta a crédito (fiado): devengado al vender, cobro salda CxC y pasa el IVA no cobrado a por pagar', () => {
    ...
    assert.ok(Number.isInteger(vc), 'venta a crédito asienta (105 Clientes / 401 Ventas + 208 IVA no cobrado)');
    assert.ok(conta.asientoVentaCredito(1001, 116) === null, 'venta a crédito idempotente');
    const cc = conta.asientoCobroCredito(1001, 116, 'efectivo');
    assert.ok(Number.isInteger(cc), 'cobro de crédito asienta (101 Caja / 105 + 208 / 209)');
    assert.ok(conta.asientoCobroCredito(1001, 116, 'efectivo') === null, 'cobro de crédito idempotente');
    const may2 = conta.libroMayor('2000-01-01', '2999-12-31');
    const saldo = (c) => { const r = may2.find(x => x.cuenta === c); return r ? r.saldo : 0; };
    assert.ok(Math.abs(saldo('105')) < 0.01, 'CxC (105) queda saldada tras el cobro');
    assert.ok(Math.abs(saldo('208')) < 0.01, 'IVA no cobrado (208) queda en cero tras el cobro');
    assert.ok(Math.abs(saldo('209') - (-16)) < 0.01, 'IVA por pagar (209) = 16 tras el cobro');
    const debe2 = may2.reduce((s, c) => s + c.debe, 0), haber2 = may2.reduce((s, c) => s + c.haber, 0);
    assert.ok(Math.abs(debe2 - haber2) < 0.01, 'libro mayor sigue cuadrado con el flujo de crédito');
});

// 9. Nómina fiscal: séptimo día (semana de 6 días) e incapacidad excluye días
test('nómina fiscal: séptimo día (semana de 6 días) e incapacidad excluye días', () => {
    db.exec(`
    CREATE TABLE empleados (id INTEGER PRIMARY KEY, nombre TEXT, salario_diario REAL, con_impuestos INTEGER DEFAULT 0, comision_pct REAL DEFAULT 0, username TEXT, activo INTEGER DEFAULT 1, fecha_alta TEXT, metodo_pago TEXT NOT NULL DEFAULT 'transferencia');
    ...
    CREATE TABLE nominas (id INTEGER PRIMARY KEY AUTOINCREMENT, id_empleado INTEGER, desde TEXT, hasta TEXT, horas REAL, horas_extra REAL, comisiones REAL, bruto REAL, isr REAL, imss REAL, neto REAL, prima_dominical REAL DEFAULT 0, imss_patronal REAL DEFAULT 0, septimo_dia REAL DEFAULT 0, estatus TEXT DEFAULT 'calculada', pagada_en TEXT, pagada_por TEXT, UNIQUE(id_empleado, desde, hasta));
    `);
    ...
    assert.ok(nom && nom.septimo_dia === 400, 'séptimo día = 1 salario por semana de 6 días (dio ' + (nom && nom.septimo_dia) + ')');
    assert.ok(nom.prima_dominical === 0, 'sin domingo trabajado → prima dominical 0');
    assert.ok(nom.bruto === 2800, 'bruto = 6×día + séptimo día (esperado 2800, dio ' + nom.bruto + ')');
    ...
    assert.ok(nom2.bruto === 1600 && nom2.septimo_dia === 0, 'incapacidad excluye 2 días (bruto 1600, séptimo 0; dio ' + nom2.bruto + '/' + nom2.septimo_dia + ')');
});

// 10. Asiento de nómina con retenciones + IMSS patronal (601/102/211/210) cuadra
test('asiento de nómina con retenciones + IMSS patronal (601/102/211/210) cuadra', () => {
    ...
    assert.ok(Math.abs(dN - hN) < 0.01, 'libro mayor cuadra tras asentar nómina con retenciones/patronal');
    assert.ok(!!mayN.find(x => x.cuenta === '210') && !!mayN.find(x => x.cuenta === '211'), 'asiento de nómina usa 210 (IMSS patronal) y 211 (retenciones)');
});
