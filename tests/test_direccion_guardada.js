...
// Ejecutar: node tests/test_direccion_guardada.js
'use strict';
...
console.log('\nSuite: Reuso de dirección guardada (addressFlow.js / orderFlow.js / _shared.js, módulos reales)\n');
...
// ── 1. Sin dirección guardada → va directo a ASK_NOMBRE, como antes ───────
{
    ...
    ok(/nombre completo/i.test(prompt), 'sin dirección previa, el prompt pide nombre completo directamente');
    const ses = sessionMgr.getSession(userId);
    ok(ses.paso_actual === S.ASK_NOMBRE, 'la sesión avanza a ASK_NOMBRE cuando no hay dirección guardada');
}
...
// ── 2. Con dirección guardada → ofrece reusarla en vez de ASK_NOMBRE ──────
{
    ...
    ok(/dirección guardada/i.test(prompt), 'con dirección previa, ofrece reusarla');
    ok(prompt.includes('Av. Siempre Viva 123') && prompt.includes('Casa azul'), 'el prompt muestra la dirección guardada completa');

    const ses = sessionMgr.getSession(userId);
    ok(ses.paso_actual === S.CONFIRM_DIR_GUARDADA, 'la sesión avanza a CONFIRM_DIR_GUARDADA en vez de ASK_NOMBRE');
    ok(ses.data.direccionGuardada && ses.data.direccionGuardada.calle === 'Av. Siempre Viva 123', 'la dirección guardada queda disponible en session.data');
}
...
//      avanza a CONFIRM_ORDER con los datos de la dirección ya mergeados ──
{
    ...
    const resp = addressFlow.handle(ctx);

    Promise.resolve(resp).then((texto) => {
        ok(texto.includes('Calle Falsa 456') && texto.includes('Confirmar y pagar'), 'opción 1 construye el resumen del pedido con la dirección guardada');
        const ses2 = sessionMgr.getSession(userId);
        ok(ses2.paso_actual === S.CONFIRM_ORDER, 'tras aceptar, la sesión avanza a CONFIRM_ORDER (igual que captura manual)');
        ok(ses2.data.calle === 'Calle Falsa 456' && ses2.data.referencia === 'Portón negro', 'los datos de la dirección guardada quedan mergeados en session.data');
        ok(!ses2.data.direccionGuardada, 'direccionGuardada no queda colgando en session.data una vez usada');
        finalizar();
    });
}
...
// ── 4. Rechazar la dirección guardada (opción 2) vuelve a ASK_NOMBRE ──────
{
    ...
    Promise.resolve(resp).then((texto) => {
        ok(/nombre completo/i.test(texto), 'opción 2 vuelve a pedir el nombre completo desde cero');
        const ses2 = sessionMgr.getSession(userId);
        ok(ses2.paso_actual === S.ASK_NOMBRE, 'la sesión retrocede a ASK_NOMBRE al rechazar la dirección guardada');
        ok(!ses2.data.direccionGuardada, 'direccionGuardada se descarta al elegir "usar otra"');
        finalizar();
    });
}
...
//      dirección guardada en vez de ir directo a ASK_NOMBRE ───────────────
{
    ...
    const resp = orderFlow.handle(ctx);
    Promise.resolve(resp).then((texto) => {
        ok(/dirección guardada/i.test(texto) && texto.includes('Integración 5'), 'orderFlow.js (DELIVERY soloEnvio) ofrece la dirección guardada real, no solo el ASK_NOMBRE hardcoded');
        const ses = sessionMgr.getSession(userId);
        ok(ses.paso_actual === S.CONFIRM_DIR_GUARDADA, 'orderFlow.js deja la sesión en CONFIRM_DIR_GUARDADA, no en ASK_NOMBRE');
        finalizar();
    });
}

let pendientes = 3; // promesas 3, 4 y 5
function finalizar() {
    pendientes--;
