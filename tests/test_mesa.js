...
const ctx = over => ({ userId: USER, action: (over.raw || '').toLowerCase().trim(), raw: '', data: {}, tel: '5218110000000', ...over,
    get action() { return (this.raw || '').toLowerCase().trim(); } });

(async () => {
    await t('iniciar: pregunta el número de mesa y deja MESA_ABRIR', () => {
        const r = mesa.iniciar(USER, {});
        assert(/n[uú]mero de mesa/i.test(r));
        assert.strictEqual(sm.getSession(USER).paso_actual, S.MESA_ABRIR);
    });

    await t('MESA_ABRIR: abre la mesa y muestra el menú → MESA_CONSUMO', async () => {
    ...
    await t('MESA_CONSUMO: agrega un platillo a mesa_items', async () => {
        const s = sm.getSession(USER);
    ...
    await t('MESA_CONSUMO: "cuenta" cierra el consumo, marca cocina y vuelve a MENU', async () => {
        const s = sm.getSession(USER);
    ...
    await t('retomar mesa: mismo número reusa la mesa abierta (no crea otra)', async () => {
    ...
    console.log('\n' + ok + '/5 OK — consumo en mesa por WhatsApp.');
