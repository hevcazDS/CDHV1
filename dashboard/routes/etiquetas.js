...
module.exports = function etiquetasRoutes(req, res, p, u, ctx, next) {
    const { db, json, readBody, log, registrarErrorDB, SECURITY_HEADERS } = ctx;
    ...
    if (p === '/api/etiquetas/pendientes-count' && req.method === 'GET') {
    ...
    if (req.method === 'PUT' && p.match(/^\/api\/etiquetas\/\d+$/)) {
    ...
    // de requireSession (ver dashboard/server.js, gate global de /api/*);
    // el nombre se valida contra _RE_ARCHIVO antes de tocar el filesystem.
    if (p.startsWith('/api/imagenes_clientes/') && req.method === 'GET') {
        const archivo = decodeURIComponent(p.slice('/api/imagenes_clientes/'.length));
    ...
    if (p === '/api/logs_error' && req.method === 'GET') {
