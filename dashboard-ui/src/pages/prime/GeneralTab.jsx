...
// Régimen fiscal del negocio (solo Prime). Referencia + congruencia: RESICO y
// persona física usan IVA base flujo de efectivo (lo que hace el sistema).
const REGIMENES = [
...
// Facturapi/Facturama = KEY-ONLY: el PAC guarda el CSD, aquí solo va la API key.
// finkok/otro = modelo CSD propio (subir .cer/.key). Facturapi primero: es el
// más simple ("solo la key") y el timbrado real YA está conectado.
const PACS = [
...
// Pasarela de pago (key-only + modo demo) — solo Prime. Con la key del proveedor
// (Stripe/Mercado Pago) se generan links reales; el modo demo simula el link para
// presentarle el sistema a un cliente sin contratar la pasarela.
const PASARELAS = [
